...
impl ExternalRecord {
    ...
    /// from an untrusted or adversarial source. The `Deserialize` impl
    /// (behind the `serde` feature) inherits the same trust boundary, since
    /// it is built on this constructor.
